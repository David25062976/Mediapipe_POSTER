...
def parse_args():
    ...
    parser.add_argument('--image_folder', type=str, default='/home/Dataset/customer_ori2/out/English Vocabulary at a CLOTHING STORE - Real Engl/English Vocabulary at a CLOTHING STORE - Real Engl-3', help='Path to video or image folder')
    ...
    parser.add_argument('--csv_label', type=str, default='', help='Path to human annotated labels CSV')
    parser.add_argument('--modeltype', type=str, default='large', help='Model type')
    parser.add_argument('--gpu', type=str, default='1', help='GPU ID')
    # 新增 boolean 參數來控制是否使用 MTCNN
    parser.add_argument('--use_mtcnn', type=lambda x: str(x).lower() in ['true', '1', 'yes', 't', 'y'], default=False, help='Set to True to use MTCNN, False to skip it and rely on MediaPipe (default: True)')
    return parser.parse_args()

def load_human_labels(csv_path):
    if not os.path.exists(csv_path):
        print(f"Warning: CSV file not found at {csv_path}. Proceeding without ground truth.")
        return None
    ...
    with open(csv_path, 'r', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            ...
            val = row[0].strip()
            if val.isdigit():
                original_label = int(val)
                if original_label in MAP_3_TO_8:
                    labels.append(MAP_3_TO_8[original_label])
                else:
                    print(f"Warning: Unknown label {original_label} in CSV. Ignored.")
                    
    if not labels:
        print("Warning: CSV file is empty or no valid numeric labels found in the first column.")
        
    return labels

def calculate_accuracy(preds, human_labels):
    if not human_labels:
        return
    
    len_preds = len(preds)
    len_human = len(human_labels)
    
    if len_preds != len_human:
        print(f"\n[WARNING] Frame count mismatch! Model frames: {len_preds}, Human labels: {len_human}.")
        print("[WARNING] Accuracy will only be calculated on the intersecting frames.")
    
    min_len = min(len_preds, len_human)
    correct = 0
    valid_frames = 0
    ...
    for i in range(min_len):
        if not np.isnan(preds[i]):
            valid_frames += 1
            if int(preds[i]) == human_labels[i]:
                correct += 1
                
    if valid_frames > 0:
        acc = correct / valid_frames
        print(f"\n=== Evaluation Result ===")
        print(f"Total Intersecting Frames: {min_len}")
        print(f"Valid Predicted Frames: {valid_frames} (Excluded NaNs)")
        print(f"Correct Predictions: {correct}")
        print(f"Accuracy: {acc:.2%} ({correct}/{valid_frames})\n")
    else:
        print("\n[WARNING] No valid predictions to calculate accuracy.\n")
...
def process_video_or_folder(args, model_8c, model_3c, device, transform):
    ...
    # 根據 args.use_mtcnn 決定是否實例化 MTCNN，節省資源
    mtcnn = MTCNN(keep_all=False, device=device) if args.use_mtcnn else None
    print(f"MTCNN Enabled: {args.use_mtcnn}")
    ...
    while True:
        ...
        if found_target and face_crop.size > 0:
            ...
            if args.use_mtcnn:
                ...
                yaw = get_pure_yaw_rgb(face_rgb, face_mesh)
                # 不使用 MTCNN 時，若 mediapipe 抓不到特徵點，即判定為後腦杓
                if yaw is None:
                    is_back_head = True
            ...
            if is_back_head:
                ...
                # 若抓不到 yaw (發生在 MTCNN 偵測到但 mediapipe 失敗的情況)，給予一個極大值代表側臉
...
def plot_results(x, y, probs_matrix, save_dir, human_labels=None):
    ...
    ax1.plot(x, y, linestyle='-', marker='o', markersize=3, linewidth=1.5, color='black', label='Model Prediction')
    ...
    if human_labels is not None:
        ...
        if len_y != len_human:
            print(f"[WARNING] Plotting mismatch: Model output ({len_y}) vs Human labels ({len_human}). Plotting up to {len_y} frames.")
        
        plot_human = human_labels[:len_y]
        if len(plot_human) < len_y:
            plot_human.extend([np.nan] * (len_y - len(plot_human)))
            
        ax1.plot(x, plot_human, linestyle='--', marker='x', markersize=4, linewidth=1.5, color='red', label='Human Ground Truth')
...
if __name__ == "__main__":
    ...
    human_labels = load_human_labels(args.csv_label)
    ...
    if os.path.exists(args.image_folder):
        ...
        if len(frames) > 0:
            if human_labels:
                calculate_accuracy(preds, human_labels)
            
            plot_results(frames, preds, probs, sdir, human_labels)
