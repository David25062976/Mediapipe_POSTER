...
class Affectdataset_8class_addpicture(data.Dataset):
    def __init__(self, root, dataidxs=None, train=True, transform=None, basic_aug=False, download=False, data=None):
        self.root = root
        ...
        NAME_COLUMN = 0
        LABEL_COLUMN = 1
        df_train = pd.read_csv(os.path.join(self.root, self.data + '.txt'), sep=' ',
                               header=None)
        df_valid = pd.read_csv(os.path.join(self.root, self.data + '.txt'), sep=' ',
                               header=None)
        if self.train:
            dataset = df_train
        else:
        ...
        for f in file_names:
            ...
            # f = f + "_aligned.jpg"
            if  self.train:
                path = os.path.join('/home/Dataset/AffectNetFull/Manually_Annotated_Images/', f)
            else:
                path = os.path.join('/home/Dataset/AffectNetFull/Manually_Annotated_Images/', f)
