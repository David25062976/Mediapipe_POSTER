...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='vivit', help='dataset')
    ...
    parser.add_argument('--lr', type=float, default=0.04, help='Initial learning rate for sgd.')
...
def run_training():
    ...
    elif args.dataset == "vivit":
        datapath = './data/vivit/'
        num_classes = 6
        train_dataset = VivitDataSet(datapath, train=True, transform=data_transforms, basic_aug=True)
        val_dataset = VivitDataSet(datapath, train=False, transform=data_transforms_val)
        model = pyramid_trans_expr(img_size=224, num_classes=num_classes, type=args.modeltype, num_frames=32)
