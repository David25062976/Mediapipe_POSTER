...
def plot_confusion_matrix(cm, labels, title, acc, save_path=None):
    plt.figure(figsize=(8, 6))
    sns.heatmap(cm, annot=True, fmt='.2f', cmap='Blues', xticklabels=labels, yticklabels=labels)
    plt.xlabel('Predicted')
    plt.ylabel('True')
    plt.title(f'{title} Confusion Matrix (Acc={acc:.4f})')
    if save_path:
        ...
        with torch.no_grad():
            ...
            if val_acc > 0.65 and val_acc > best_acc:
                ...
                torch.save({'iter': i,
                           ...
                           os.path.join('./checkpoint', f"epoch{i}_acc{val_acc:.4f}.pth"))
                print('Model saved.')

                # === 計算混淆矩陣並畫圖 ===
                cm = confusion_matrix(gt_labels, pre_labels)
                cm = cm.astype('float') / cm.sum(axis=1)[:, np.newaxis]  # 轉換成百分比 (0~1)
                cm = np.round(cm * 100, 2)  # 變成百分比並保留 2 位小數
                os.makedirs('./confusion_matrix', exist_ok=True)  # 確保資料夾存在

                if args.dataset == "rafdb":
                    labels_name = ['SU', 'FE', 'DI', 'HA', 'SA', 'AN', "NE"]
                    plot_confusion_matrix(cm, labels_name, 'RAF-DB', val_acc,
                                          save_path=f"./confusion_matrix/epoch{i}_rafdb.png")

                elif args.dataset == "affectnet":
                    labels_name = ['NE', 'HA', 'SA', 'SU', 'FE', 'DI', "AN"]
                    plot_confusion_matrix(cm, labels_name, 'AffectNet', val_acc,
                                          save_path=f"./confusion_matrix/epoch{i}_affectnet.png")

                elif args.dataset == "affectnet8class":
                    labels_name = ['NE', 'HA', 'SA', 'SU', 'FE', 'DI', "AN", "CO"]
                    plot_confusion_matrix(cm, labels_name, 'AffectNet_8class', val_acc,
                                          save_path=f"./confusion_matrix/epoch{i}_affectnet8.png")
                                          
                elif args.dataset == "affectnet8class_2":
                    labels_name = ['NE', 'HA', 'SA', 'SU', '(LI)', 'DI', "AN", "(HES)"]
                    plot_confusion_matrix(cm, labels_name, 'AffectNet_8class_2', val_acc,
                                          save_path=f"./confusion_matrix/epoch{i}_affectnet8.png")
