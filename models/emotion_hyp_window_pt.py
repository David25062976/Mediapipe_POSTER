...
# ==========================================================
# 統一的單幀權重載入函數 (供 train.py 呼叫)
# ==========================================================
def load_single_frame_weights(model, checkpoint_path):
    """
    載入單幀模型 (pyramid_trans_expr) 的預訓練權重，並印出詳細的比對報告。
    """
    print(f"\n{'='*20} 載入單幀預訓練權重 {'='*20}")
    print(f"來源檔案: {checkpoint_path}")
    ...
    new_state_dict = {}
    matched_layers = []
    size_mismatch_layers = []
    unexpected_keys = []
    skipped_head_keys = []

    # 1. 掃描 Checkpoint 中的所有權重
    for k, v in state_dict.items():
        ...
        if name in model_dict:
            ...
            if model_dict[name].size() == v.size():
                ...
                size_mismatch_layers.append((name, v.size(), model_dict[name].size()))
        else:
            # 模型中沒有這個層 (可能是舊版的殘留物)
            unexpected_keys.append(name)

    # 2. 檢查目前 Model 中，有哪些權重是 Checkpoint 沒給的 (會被隨機初始化)
    ...
    # ==========================================
    print(f"\n[ 載入總結 Summary ]")
    print(f"✅ 成功載入 (Matched)   : {len(matched_layers)} 個張量")
    print(f"⏭️ 刻意跳過 (Skipped)   : {len(skipped_head_keys)} 個張量 (舊分類頭)")
    print(f"❌ 維度不符 (Mismatch)  : {len(size_mismatch_layers)} 個張量")
    print(f"❓ 未知權重 (Unexpected): {len(unexpected_keys)} 個張量 (存檔有，但現有模型沒有)")
    print(f"⚠️ 缺失權重 (Missing)   : {len(missing_keys)} 個張量 (現有模型有，但存檔沒有)")

    if size_mismatch_layers:
        print("\n[ ❌ 維度不符詳細清單 ]")
        for name, ckpt_sz, mod_sz in size_mismatch_layers:
            print(f"  - {name}: 存檔 {list(ckpt_sz)} -> 模型 {list(mod_sz)}")
            
    if missing_keys:
        print("\n[ ⚠️ 缺失權重清單 (這些層將被隨機初始化，準備進行訓練) ]")
        # 為了排版乾淨，我們可以稍微把同一個模組的歸類一下
        for name in missing_keys:
            # 重點觀察：這裡應該要印出 temporal.xxx 和 head.xxx
            print(f"  - {name}")
...
class pyramid_trans_expr_window_pt(nn.Module):
    def __init__(self, img_size=224, num_classes=7, window_size=8, type="large", freeze_list=[False, False, False], mediapipe_points=196, mediapipe_patch_size=24, temporal_type="MLP"):
        super().__init__()
        depth = 8
        if type == "small":
            depth = 4
        ...
        # ==========================================================
        #  Freeze 邏輯 (只訓練 Temporal 網路)
        # ==========================================================
        FL_freeze, IR_freeze, PF_freeze = freeze_list
        
        print("=" * 12 + "Freeze" + "=" * 12)
        if FL_freeze:
            for param in self.face_landback.parameters(): param.requires_grad = False
            print("[Info] face_landback 參數已凍結")
        if IR_freeze:
            for param in self.ir_back.parameters(): param.requires_grad = False
            print("[Info] ir_back 參數已凍結")
        if PF_freeze:
            for param in self.pyramid_fuse.parameters(): param.requires_grad = False
            for param in self.se_block.parameters(): param.requires_grad = False
            print("[Info] pyramid_fuse & se_block 參數已凍結")
        if not FL_freeze and not IR_freeze and not PF_freeze:
            print("[Info] 全部未凍結")
        print("=" * 30)
        ...
        if temporal_type == "MLP":
            self.temporal = nn.Sequential(
                nn.Linear(self.window_size * 1024, 1024),
                nn.ReLU(),
                nn.Dropout(p=0.5), 
                nn.Linear(1024, 512), 
                nn.ReLU(),
            ...
            self.temporal = nn.Sequential(
                ...
                nn.BatchNorm2d(16),
                nn.ReLU(),
                nn.MaxPool2d(2), # W變 W/2, 1024變 512
            ...
            encoder_layer = nn.TransformerEncoderLayer(d_model=1024, nhead=4, dim_feedforward=1024, batch_first=True)    # nhead減少, dim_feedforward=1024
            self.temporal = nn.TransformerEncoder(
                encoder_layer, 
                num_layers=2,    # num_layers 減少
                enable_nested_tensor=False
            )
            
            # 位置編碼 (Positional Embedding)
            # 設一個夠大的 max_len (例如 100)，forward 時再切
            self.pos_embedding = nn.Parameter(torch.randn(1, 100, 1024))
            
            # Transformer 輸出後通常接 Global Average Pooling，所以維度保持 512
            linear_input_dim = 1024


        self.head = ClassificationHead(input_dim=linear_input_dim, target_dim=self.num_classes)

    def forward(self, x, coords_478):
        # 1. 將 DataLoader 進來的 [B, C, W, H, Wd] 轉成 [B, W, C, H, Wd]
        x = x.permute(0, 2, 1, 3, 4).contiguous()
        B, W, C, H, Wd = x.shape

        # ==========================================================
        #  🚀 GPU 效能優化核心：Batch-Time Flattening
        #  把 B 和 W 壓平，一次性送入網路，消除 for 迴圈的瓶頸
        # ==========================================================
        x_flat = x.view(B * W, C, H, Wd) 
        coords_flat = coords_478.view(B * W, 478, 3)

        # 2. 一次性提取整個 Sequence 的特徵
        x_face = self.face_landback(x_flat, coords_flat) # [B*W, 49, 1024]
        x_ir = self.ir_back(x_flat)                      # [B*W, 49, 1024]

        # 3. 一次性進行特徵融合
        y_hat = self.pyramid_fuse(x_ir, x_face)          # [B*W, 512]
        y_hat = self.se_block(y_hat)                     # [B*W, 512]

        # 4. 把平坦的維度重新塑形回時序序列 [B, W, 512]
        y_seq = y_hat.view(B, W, 1024)

        # ==========================================================
        #  時序融合 (Temporal Fusion)
        # ==========================================================
        if self.temporal_type == "MLP":
            y_seq_flat = y_seq.view(B, self.window_size * 1024)
            y_fused = self.temporal(y_seq_flat)
            out = self.head(y_fused)

        elif self.temporal_type == "CNN":
            cnn_in = y_seq.unsqueeze(1) # [B, 1, W, 512]
            ...
            y_fused = trans_out.mean(dim=1)  # Mean Pooling
            out = self.head(y_fused)
