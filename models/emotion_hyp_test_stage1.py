...
class pyramid_trans_expr(nn.Module):
    def __init__(self, img_size=224, num_classes=7, window_size=8, type="large", freeze=False):
        super().__init__()
        depth = 8
        if type == "small":
            depth = 4
        ...
        if freeze :
            # 定義儲存和載入路徑
            load_path = './models/pretrain/pyramid_fuse_acc0914.pth'

            # 載入儲存的 state_dict
            loaded_state_dict = torch.load(load_path)

            # 載入權重到特定的模組 (pyramid_fuse)
            # self.pyramid_fuse.load_state_dict(loaded_state_dict, strict=False)
            # print("pyramid_fuse 權重已成功載入。")

            for param in self.pyramid_fuse.parameters():
                param.requires_grad = False
            print("pyramid_fuse 模組的權重已凍結。")

        self.se_block = SE_block(input_dim=512)
        self.head = ClassificationHead(input_dim=512, target_dim=self.num_classes)    # MLP=1024, CNN=128
