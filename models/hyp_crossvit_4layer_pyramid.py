...
class Attention_lm(nn.Module):    # ? 交換q的好處在哪
    def __init__(self, dim, in_chans, q_chanel, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.num_heads = num_heads
        self.img_chanel = in_chans + 1
...
class PyramidBlock(nn.Module):
    ...
    def __init__(self, dim, in_chans, q_chanel, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.block_l = Block(    # ? 金字塔多幾層或少幾層有什麼差別
        ...
        self.block_xs = Block(
            dim=dim//8, in_chans=in_chans, q_chanel=q_chanel, num_heads=num_heads,
            mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop, attn_drop=attn_drop,
            drop_path=drop_path, norm_layer=norm_layer, act_layer=act_layer)
        ...
        self.upsample_xs = nn.ConvTranspose1d(n_channels, n_channels, kernel_size=2, stride=2)
    ...
    def forward(self, x):
        ...
        x_xs = self.block_xs(x_xs)

        x_s = x_s + self.upsample_xs(x_xs)
...
class HyVisionTransformer(nn.Module):
    ...
    def __init__(self, in_chans=49, q_chanel = 49, num_classes=1000, embed_dim=512, depth=12,
        ...
        # embed_dim 輸入維度大小
        # depth = 8 設定有8個金字塔，每個金字塔的drop_path依序遞增 # emotion_hyp
        # num_heads 在Attention_img和Attention_lm設定Multi-Head Attention為8
        # mlp_ratio = 2. 在block裡設定隱藏層的擴張倍率為2 # emotion_hyp
        # qkv_bias 在Attention_img和Attention_lm設定 self.kv = nn.Linear(dim, dim * 2, bias=qkv_bias) 是否有可訓練的bias term
        # drop_rate 在Attention_img和Attention_lm設定Dropout(proj_drop)，避免attn overfitting
        # attn_drop_rate 在Attention_img和Attention_lm設定Dropout(attn_drop)，避免x_img或x_lm overfitting
        # drop_path_rate = 0.1 與depth算每個金字塔的drop_path，ex: [0.0, 0.014285714, 0.028571428, 0.042857143, 0.057142857, 0.071428571, 0.085714286, 0.1]    # emotion_hyp
        super().__init__()
        self.num_classes = num_classes
        self.in_chans = in_chans
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)    # 正規化參數輸入，除以1e-6避免除以0
        ...
        self.downsample_xs = nn.Conv1d(n_channels, n_channels, kernel_size=8, stride=8)

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        self.blocks = nn.Sequential(*[    # ? 找nn.Sequential和*的資訊傳輸方法
            PyramidBlock(
    ...
    def forward(self, x, x_lm):
        ...
        new_x_xs = self.downsample_xs(new_x)
        new_x_in = [new_x_l, new_x_m, new_x_s, new_x_xs]
        #############################
