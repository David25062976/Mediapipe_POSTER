...
from .hyp_crossvit_affect_attention_map_p1 import *
...
class PatchFeatureEncoder478_14(nn.Module):
    ...
    def forward(self, x, num_points=478):
        feat = self.conv(x)                        # [B * N, 256, 1, 1]
        feat = feat.view(feat.size(0), -1)         # [B * N, 256]
        
        # 直接投影，不做任何 49 點的壓縮
        return self.fuse_projection(feat)          # [B * N, 1024]
...
class MediaPipeFeatureExtractor(nn.Module):
    def __init__(self, output_dim=1024, points=196, patch_size=24):
        ...
        if self.points == 478:
            ...
            # 根據網頁區域 (Eyes, Eyebrows, Lips, Jaw, Cheeks) 整理的左右對稱表
    ...
    def forward(self, x, coords_478):
        ...
        # ★ 這裡非常重要！動態根據目前的點數 (num_points) 來 view
...
class pyramid_trans_expr(nn.Module):
    ...
    def forward(self, x, coords_478, return_attention=False):
        # 1. 取得臉部特徵 [B, N, 1024] (N 可能是 478，或是動態 Drop 之後的點數)
        x_face = self.face_landback(x, coords_478)
