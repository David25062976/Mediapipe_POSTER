...
class pyramid_trans_expr(nn.Module):
    def __init__(self, img_size=224, num_classes=7, type="large", freeze=False, mediapipe_points=196, mediapipe_patch_size=24):
        ...
        self.pyramid_fuse_ir = HyVisionTransformer(in_chans=49, q_chanel = 49, embed_dim=1024,
                                             depth=depth, num_heads=8, mlp_ratio=2.,
                                             drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1)

        self.se_block_ir = SE_block(input_dim=1024)
        self.head_ir = ClassificationHead(input_dim=1024, target_dim=self.num_classes)

        self.pyramid_fuse_face = HyVisionTransformer(in_chans=49, q_chanel = 49, embed_dim=1024,
                                             depth=depth, num_heads=8, mlp_ratio=2.,
                                             drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1)

        self.se_block_face = SE_block(input_dim=1024)
        self.head_face = ClassificationHead(input_dim=1024, target_dim=self.num_classes)

    def forward(self, x, coords_478):
        x_face = self.face_landback(x, coords_478) # 往下傳遞
        x_ir = self.ir_back(x)

        y_hat_ir = self.pyramid_fuse_ir(x_ir, x_ir)
        y_hat_ir = self.se_block_ir(y_hat_ir)
        y_feat_ir = y_hat_ir
        out_ir = self.head_ir(y_hat_ir)
        
        y_hat_face = self.pyramid_fuse_face(x_face, x_face)
        y_hat_face = self.se_block_face(y_hat_face)
        y_feat_face = y_hat_face
        out_face = self.head_face(y_hat_face)

        y_feat = (y_feat_ir + y_feat_face) / 2
        out = (out_ir + out_face) / 2
