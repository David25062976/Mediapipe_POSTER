...
def run_training():
    ...
    for i in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            ...
            if val_acc > 0.67 and val_acc > best_acc:
                torch.save({'iter': i,
                            'model_state_dict': model.module.pyramid_fuse.state_dict(),
                            'optimizer_state_dict': optimizer.state_dict(), },
                           os.path.join('./checkpoint/HyVisionTransformer', "epoch" + str(i) + "_acc" + str(val_acc) + ".pth"))
                print('Model saved.')
            if val_acc > best_acc:
                best_acc = val_acc
