...
def parse_args():
    ...
    parser.add_argument('--checkpoint_window', type=str,  default='/home/lab702/POSTER/checkpoint/window_20260119-152320_MLP/epoch16_acc0.9038.pth', help='Path to the trained model .pth file')
    parser.add_argument('--yolo_path', type=str, default='yolov8n-face.pt', help='Path to yolov8 face/nano model')
    ...
    parser.add_argument('--face_crop', action='store_true', default=False, help='Apply face crop (Recommended if trained with it)')
    ...
    def crop(self, image, best_crop_coordinate):
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                # 計算距離分數
                dist = (face_center_x - img_center_x)**2 + (face_center_y - img_center_y)**2
...
def process_and_predict(model, folder_path, args, window_size, stride, device):
    ...
    # 2. 滑動視窗迴圈
    # range(start, stop, step) -> step = stride
    for i in range(0, total_frames - window_size + 1, stride):
        window_paths = full_paths[i : i + window_size]
        
        # 準備一個 batch 的資料
        window_imgs = []
        best_crop_coordinate = None # 每個 Window 重置一次 crop 座標 (為了穩定性)
        
        for path in window_paths:
            img_bgr = cv2.imread(path)
            if img_bgr is None:
                img_bgr = np.zeros((224, 224, 3), dtype=np.uint8) # 避免壞圖 crash
        ...
        # 轉置 -> (C, T, H, W) 符合模型輸入
        data_tensor = data_tensor.permute(1, 0, 2, 3).unsqueeze(0) # 增加 Batch 維度 -> (1, C, T, H, W)
        ...
        # 紀錄結果
        # 將這個 Window 的結果「廣播」給這個 Window 內的所有 Frame (或根據需求只畫中心點)
        # 這裡示範：將這 8 幀都標記為同一個預測結果
        # current_indices = list(range(i + 1, i + window_size + 1)) # Frame number start from 1
        
        # frame_indices.extend(current_indices)
        # results.extend([label] * window_size)

        # 紀錄結果
        # 只記錄 Window 的最後一幀 (Representing the prediction at the end of the window)
        last_frame_index = i + window_size  # 因為 i 是起始點(0-based)，加上長度即為該 Window 最後一幀的 1-based index
        
        frame_indices.append(last_frame_index) # 使用 append 加入單個數值
        results.append(label)                  # 使用 append 加入單個標籤
...
def plot_graph(x, y, save_path='window_result.png'):
    plt.figure(figsize=(15, 6))
    
    # 繪製階梯圖 (Step plot) 比較適合時序分類
    plt.step(x, y, where='mid', color='b', linewidth=1.5)
    plt.plot(x, y, 'o', markersize=3, color='r', alpha=0.3) # 標出每個點
    
    plt.title("Sequence Classification Result (Window Based)")
    plt.xlabel("Frame Number")
    plt.ylabel("Class ID")
    plt.yticks(range(0, 3), labels=["dislike", "hesitate", "like"]) # 假設 3 類
...
def draw_window(args, window_size, stride):
    ...
    if os.path.exists(args.image_folder):
        frames, preds = process_and_predict(model, args.image_folder, args, window_size, stride, device)
        ...
        if frames:
            ...
            save_path = os.path.join(save_dir, f'result_window{window_size}{stride}.png')
            plot_graph(frames, preds, save_path)
        else:
