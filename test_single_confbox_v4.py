...
# 將 3 類的 Index 映射到 8 類的 Index (4: Like, 5: Hesitate, 7: Dislike)，以利繪圖與對比
...
def parse_args():
    ...
    parser.add_argument('--image_folder', type=str, default='/home/Dataset/customer_ori2/out/A Simple Shopping Haul Turned Into Online Drama/Haul-3', help='Path to video or image folder')
    ...
    parser.add_argument('--csv_label', type=str, default='out_videos_label.csv', help='Path to human annotated labels CSV')
    parser.add_argument('--modeltype', type=str, default='large', help='Model type')
    parser.add_argument('--gpu', type=str, default='1', help='GPU ID')
    return parser.parse_args()

def load_human_labels(csv_path):
    """讀取 CSV 檔第一列(Column直的)，並將 0, 1, 2 轉換為 8 類的 index 4, 5, 7"""
    if not os.path.exists(csv_path):
        print(f"Warning: CSV file not found at {csv_path}. Proceeding without ground truth.")
        return None
    
    labels = []
    # 使用 utf-8-sig 可以避免 Windows 儲存 CSV 時可能產生的 BOM 亂碼問題
...
def calculate_accuracy(preds, human_labels):
    ...
    for i in range(min_len):
        # 排除模型沒有給出預測的幀數（例如後腦杓或未偵測到人臉被設為 np.nan）
