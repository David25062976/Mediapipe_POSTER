...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='affectnet8class_mediapipe', help='dataset')
    ...
    parser.add_argument('--early_stop_patience', type=int, default=30, help='Tolerating no progress for n epochs.')
...
def run_training():
    ...
    if args.dataset == "rafdb":
        ...
        datapath = './data/AffectNet_customer_2class/'
        train_dataset_path='train_set_mediapipe'
        valid_dataset_path='valid_set_mediapipe'
        class_names = ['Neutral', 'Happy', 'Sad', 'Surprise', 'interested', 'thinking', 'Anger', 'pass']
        num_classes = len(class_names)
        train_dataset = Affectdataset_8class_2(datapath, train=True, transform=data_transforms, basic_aug=True, train_dataset_path=train_dataset_path, valid_dataset_path=valid_dataset_path)
        val_dataset = Affectdataset_8class_2(datapath, train=False, transform=data_transforms_val, train_dataset_path=train_dataset_path, valid_dataset_path=valid_dataset_path)
        model = pyramid_trans_expr(img_size=224, num_classes=num_classes, type=args.modeltype, freeze=args.freeze)

    elif args.dataset == "affectnet8class_ori":
        datapath = './data/AffectNet_customer/'
        train_dataset_path='train_set/train_set_AffectNet'
        valid_dataset_path='valid_set/valid_set_AffectNet'
        ...
        datapath = './data/AffectNet8c_Shopping'
        train_dataset_path='train_set'
        valid_dataset_path='valid_set'
